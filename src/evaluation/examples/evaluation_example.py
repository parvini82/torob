...
from evaluation import EntityTagEvaluator, EvaluationConfig, ModelEvaluator
...
def example_toy_sample_evaluation():
    ...
    print(
        f"Entity Coverage Rate: {results['entity_coverage']['entity_coverage_rate']:.1f}%"
    )
    print(f"Image Validity Rate: {results['image_validity']['url_validity_rate']:.1f}%")
    print(
        f"Group Diversity Score: {results['diversity_metrics']['group_diversity']:.3f}"
    )
...
def example_entity_extraction_evaluation():
    ...
    results = evaluator.evaluate_entity_extraction(
        predictions, ground_truths, "example_entities"
    )
    ...
    if "attribute_summary" in results:
        ...
        for attr_name, metrics in results["attribute_summary"].items():
            print(
                f"  {attr_name}: F1={metrics['mean_f1']:.3f}, Precision={metrics['mean_precision']:.3f}, Recall={metrics['mean_recall']:.3f}"
            )
...
def example_single_entity_evaluation():
    ...
    single_result = entity_evaluator.evaluate_single_sample(
