...
class EntityTagEvaluator:
    ...
    def __init__(self, config: EvaluationConfig, embedding_model_name: str = 'all-MiniLM-L6-v2'):
    ...
    def exact_match_score(self, predicted_entities: List[Dict], true_entities: List[Dict]) -> float:
    ...
    def partial_match_score(self, predicted_entities: List[Dict], true_entities: List[Dict]) -> Dict[str, float]:
    ...
    def attribute_level_metrics(self, predicted_entities: List[Dict], 
                               true_entities: List[Dict]) -> Dict[str, Dict[str, float]]:
        ...
        for attr_name in all_attrs:
            ...
            results[attr_name] = {
                ...
                'fp': fp,
                'fn': fn,
                'support': len(true_values)  # Number of true instances for this attribute
            }
        
        return results
    
    def semantic_similarity_score(self, predicted_entities: List[Dict], 
        ...
        for attr_name in all_attrs:
            ...
            try:
                ...
                print(f"Warning: Error calculating embeddings for attribute '{attr_name}': {e}")
    ...
    def eighty_percent_accuracy(self, predicted_entities: List[Dict], 
                               true_entities: List[Dict]) -> float:
    ...
    def evaluate_single_sample(self, predicted_entities: List[Dict], 
        ...
        results = {
            ...
            'partial_match': self.partial_match_score(predicted_entities, true_entities),
            'attribute_metrics': self.attribute_level_metrics(predicted_entities, true_entities),
            'semantic_similarity': self.semantic_similarity_score(predicted_entities, true_entities),
            'eighty_percent_accuracy': self.eighty_percent_accuracy(predicted_entities, true_entities)
        }
        return results
    ...
    def evaluate_batch(self, predictions: List[List[Dict]], 
        ...
        for i, (pred, true) in enumerate(zip(predictions, ground_truths)):
            try:
                ...
                sample_results.append({
                    ...
                    'semantic_similarity': {'semantic_precision': 0.0, 'semantic_recall': 0.0, 'semantic_f1': 0.0},
        ...
        macro_results = {
            ...
            'partial_match_precision': np.mean([r['partial_match']['precision'] for r in valid_results]),
            'partial_match_recall': np.mean([r['partial_match']['recall'] for r in valid_results]),
            'partial_match_f1': np.mean([r['partial_match']['f1'] for r in valid_results]),
            'semantic_precision': np.mean([r['semantic_similarity']['semantic_precision'] for r in valid_results]),
            'semantic_recall': np.mean([r['semantic_similarity']['semantic_recall'] for r in valid_results]),
            'semantic_f1': np.mean([r['semantic_similarity']['semantic_f1'] for r in valid_results]),
            'eighty_percent_accuracy': np.mean([r['eighty_percent_accuracy'] for r in valid_results]),
        ...
        for key in macro_results:
            if isinstance(macro_results[key], (float, np.floating)):
                macro_results[key] = round(macro_results[key], self.config.precision_digits)
        ...
        try:
            ...
            micro_semantic = self.semantic_similarity_score(all_pred_flat, all_true_flat)
        ...
        return {
            ...
            'summary': {
                ...
                'success_rate': len(valid_results) / len(sample_results) if sample_results else 0.0
            }
        }
    
    def calculate_attribute_summary(self, batch_results: Dict[str, Any]) -> Dict[str, Dict[str, float]]:
        ...
        """
        attribute_stats = defaultdict(lambda: {'precision': [], 'recall': [], 'f1': [], 'support': []})
        ...
        for attr_name, stats in attribute_stats.items():
            summary[attr_name] = {
                'mean_precision': round(np.mean(stats['precision']), self.config.precision_digits),
                'mean_recall': round(np.mean(stats['recall']), self.config.precision_digits),
                'mean_f1': round(np.mean(stats['f1']), self.config.precision_digits),
                'std_precision': round(np.std(stats['precision']), self.config.precision_digits),
                'std_recall': round(np.std(stats['recall']), self.config.precision_digits),
