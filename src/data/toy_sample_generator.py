...
class ToySampleGenerator:
    ...
    def is_outlier(self, product: Product) -> bool:
        ...
        return (
            self.get_title_length(product) >= rules["min_title_length"] or
            self.count_entities(product) >= rules["min_entity_count"]
        )
    ...
    # Sampling utilities
    def sample_from_pool(self, pool: List[Product], k: int, rng: random.Random) -> List[Product]:
    ...
    def proportional_quota(self, count: int, ratios: Dict[str, float]) -> Dict[str, int]:
        ...
        if remainder > 0:
            ...
            fractional_parts = sorted(
                ((k, raw_allocations[k] - base_allocations[k]) for k in raw_allocations),
    ...
    def generate_toy_sample(self, products: List[Product], seed: int = 42) -> List[Product]:
        ...
        # Group composition
        group_quotas = self.proportional_quota(target_size, self.config.group_composition)
        ...
        title_quotas = self.proportional_quota(target_size, {
        ...
        while len(sample) < target_size:
            ...
            core_candidates = [p for p in indices["has_core_entities"] 
                             if self.get_product_id(p) not in used_ids]
            other_candidates = [p for p in unique_products 
                              if self.get_product_id(p) not in used_ids]
    ...
    def generate_and_save_sample(self, products: List[Product], 
                                output_path: Path = None) -> List[Product]:
